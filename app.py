...
def verifyoldnotes(sample,reference_image,Note_Type,proper_image, verify_feat_old, feature_set_old, mini):
    add=0
    try:
    ...
    for iter, feature_x in enumerate(verify_feat_old):
        ...
        reference_image = cv2.imread('/Image/' + str(Note_Type)+'/000' + str(feature_x+1) + '.jpg', 0)
        try:
            matches , kp = imageMatcher(reference_image, sample)
            if matches == None or kp == None:
    ...
    if final>=0.65:
        str1 = "the note is Genuine " + str(final) + " "+str(add) 
    else:
        str1 = "the note is Fake " + str(final)+ " " +str(add) 
    #cv2.namedWindow(title, cv2.WINDOW_KEEPRATIO)
    proper_image = cv2.cvtColor(proper_image, cv2.COLOR_BGR2RGB)
    #cv2.imshow(title, proper_image) 
    return str1

def verifynewnotes( sample,reference_image,Note_Type,proper_image, verify_feat, feature_set, mini):
    add=0
    try:
        matches , kp = imageMatcher(reference_image, sample)
        if matches == None or kp == None:
...
def index():
    ...
    Note_Type = int(request.form['optradio'])
    ...
    # ============ PROPER IMAGE TO PROCESS =================
        ...
        if Note_Type==200 or Note_Type==500:
                if Note_Type==200:
                    ...
                    final = verifynewnotes(sample,reference_image,Note_Type,proper_image, verify_feat, feature_set, mini)
            ...
            if Note_Type==100 or Note_Type==50 or Note_Type==20 or Note_Type==10:
                ...
                if Note_Type==100:
                    for x_out, d_feat in enumerate(detect_feat_old):
                        ...
                        reference_image = cv2.imread('/Image/' + str(Note_Type)+'/000' + str(d_feat+1) + '.jpg', 0)
                        final = verifyoldnotes(sample,reference_image,Note_Type,proper_image, verify_feat_old, feature_set_old, mini)
